...
const cardBody = (type, card) => {
    const { fetchIcon } = useIcons();
    const {list, links, rate, fill, title, color, subtitle, text } = card
    switch (type) {
        case 'course':
            return (
                <main className="p-4">
        ...
        case 'teacher':
            return (
                <main className="flex flex-col justify-between px-4 text-base">
                  ...
                  <p className='text-center py-2 my-4 border rounded-sm'><span className="font-semibold">{rate.title}</span> {rate.text}</p>
                </main>
              );
        case 'testimonial':
            return (
